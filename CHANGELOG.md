...
- Generation Job review for every kind (US-022, #57, DEC-017): `jobs review` now accepts Plate, Object, and Creator jobs and writes one offline, script-free sheet — every distinct candidate content-identity-verified before rendering, shown at full resolution (with the exact local file as a 1:1 target) and at exactly 168px, the row size that decides legibility; the isolation section reads through the one canonical reader adoption uses (`resolveAdoptedBytes`), so it shows the matte adoption would write (engine named), marks a natively isolated Object candidate adoptable as-is, and plainly marks a candidate with no adoptable matte — while Creator review keeps its identity-anchor, face-detail, and matte evidence; tampered or missing recorded evidence fails the whole review instead of producing a partial one
