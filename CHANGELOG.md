...
- **Model selection reads one qualified capability source** (US-031/US-032, #53, DEC-018–DEC-020, TEST-011): gpt-image (GPT Image 2) is qualified reference-capable per the #52 Gateway evidence, so a Plate/Object Job with typed References keeps the gpt-image default, while an explicit incompatible selection — registry key or raw gateway id — is refused before any provider call and therefore any spend, listing every qualified compatible model derived from the registry (`referenceCapableModels`). Raw gateway ids no longer infer capability from the model-name substring; `jobs rerun` re-validates the recorded model against the current registry before reading reference bytes. Creator jobs keep the deliberate `nano-2` likeness default, now the named `CREATOR_DEFAULT_MODEL` rather than an inline literal
