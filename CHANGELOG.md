...
- Matting weights are cached under `models/` (gitignored, `THUMBY_MODEL_DIR` overrides), pinned by filename and sha-256 and verified once per process: a missing or mismatched model fails loudly with the path, the pin, and the fetch command — the pass never silently skips isolation. Adds `onnxruntime-node` (native postinstall, `trustedDependencies`) (#16)
