...
- Layer inspection and selection in the Scene author session (#59): the one render pass now also measures every resolved Layer — `RenderedLayer` entries in tree order (nested Group children and Connectors included) with browser-measured bounds (the wrapper for non-connectors; the connector's rendered path with its stroke and arrow extent) and bounds absent for hidden Layers, while the PNG stays byte-identical to an ordinary render. The author view lists every Layer exactly once — hidden ones as visibly hidden, disabled, non-selectable rows — and a single script-free radio group drives one shared selection from either indexed canvas hit/highlight boxes (exact transformed bounds over the current Render, compositing-order priority, symmetric minimum hit targets) or the listing; opening and using selection leaves the Scene bytes unchanged.
