...
- Scene author session: a Connector's listing bounds now update when its target Layers move, and a delayed older movement outcome can no longer overwrite the status of a newer one — a late older error cannot hide a newer accepted preview, and a late older success cannot hide a newer rejection, while the accepted preview, geometry, and applied revision still apply (#60).
