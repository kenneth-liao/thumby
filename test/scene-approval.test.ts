...
beforeAll(async () => {
  ...
  await writeCutout("kenny-trial-2", "trial");
...
describe("trial Creator Asset render gate", () => {
  it("rejects a scene referencing a trial Creator Asset with a layer-specific error", async () => {
    const errors = await loadErrors(scene([plateLayer(), creatorLayer("kenny-trial")]));
    expect(errors).toHaveLength(1);
    expect(errors[0]!.path).toBe("layers[1].asset");
    expect(errors[0]!.message).toMatch(/kenny-trial/);
      ...
      scene([
        {
          ...
          layers: [creatorLayer("kenny-trial")],
  ...
  it("loads a scene referencing an approved Creator Asset", async () => {
    const { resolved } = await load(scene([creatorLayer("kenny-approved")]));
    expect(resolved.assets.get("creator")!.id).toBe("kenny-approved");
  ...
  it("surfaces the approval state on the resolution — one home, no re-scan", async () => {
    const { resolved } = await load(scene([plateLayer(), creatorLayer("kenny-trial")]), {
  ...
  it("the experimental override permits trial rendering", async () => {
    const { resolved } = await load(scene([creatorLayer("kenny-trial")]), {
      allowTrialCreator: true,
    });
    expect(resolved.assets.get("creator")!.id).toBe("kenny-trial");
  });
...
describe("approveCutout", () => {
  it("records the approver decision on the trial asset", async () => {
    const dir = path.join(fix.libRoot, "cutouts", "kenny-trial");
    const before = await readFile(path.join(dir, "cutout.png"));
    const meta = await approveCutout(fix.libRoot, "kenny-trial", {
      approvedBy: "Kenneth Liao",
      approvedAt: "2026-02-14T10:00:00.000Z",
      approvalNote: "likeness verified against anchors",
    });
    expect(meta.approval).toBe("approved");
    expect(meta.approvedBy).toBe("Kenneth Liao");
    ...
    await load(scene([creatorLayer("kenny-trial")]));
  });
  ...
  it("refuses an already-approved asset instead of silently re-deciding", async () => {
    const dir = path.join(fix.libRoot, "cutouts", "kenny-approved");
    const before = JSON.parse(await readFile(path.join(dir, "meta.json"), "utf8"));
    await expect(
      approveCutout(fix.libRoot, "kenny-approved", {
  ...
  it("rejects a decision without an approver", async () => {
    await expect(
      approveCutout(fix.libRoot, "kenny-trial-2", { approvedBy: "", approvedAt: "2026-02-14T10:00:00.000Z" }),
    ).rejects.toThrow(/approver/i);
  });

  it("approving one candidate leaves unrelated assets untouched", async () => {
    const other = path.join(fix.libRoot, "cutouts", "kenny-trial-2", "meta.json");
...
describe("scene cli approval enforcement", () => {
  ...
  it("validate rejects a trial Creator Asset reference", async () => {
    const file = await sceneFile("trial.json", [creatorLayer("kenny-trial-2")]);
    ...
    expect(errors[0]!.message).toMatch(/kenny-trial-2/);
  });

  it("validate accepts an approved Creator Asset reference", async () => {
    const file = await sceneFile("approved.json", [creatorLayer("kenny-approved")]);
    const { exitCode, output } = await cli(["validate", file]);
  ...
  it("render refuses a trial Creator Asset with the layer-specific error", async () => {
    const file = await sceneFile("render-trial.json", [creatorLayer("kenny-trial-2")]);
  ...
  it("--experimental renders a trial Creator Asset, marked non-final", async () => {
    const file = await sceneFile("experimental.json", [creatorLayer("kenny-trial-2")]);
  ...
  it("--experimental on an all-approved scene writes no undefined warning and rerenders", async () => {
    ...
    const file = await sceneFile("exp-approved.json", [creatorLayer("kenny-approved")]);
    const { exitCode, output } = await cli(["render", file, "--experimental"]);
    expect(exitCode).toBe(0);
  ...
  it("a plain render's manifest is not marked experimental", async () => {
    const file = await sceneFile("plain.json", [creatorLayer("kenny-approved")]);
    const { exitCode, output } = await cli(["render", file]);
    expect(exitCode).toBe(0);
  ...
  it("rerender honors a recorded experimental manifest without the flag", async () => {
    const file = await sceneFile("rerender-trial.json", [creatorLayer("kenny-trial-2")]);
    const { exitCode, output } = await cli(["render", file, "--experimental"]);
    expect(exitCode).toBe(0);
  ...
  it("rerender rejects a scene whose trial reference has no recorded override", async () => {
    ...
    const file = await sceneFile("forged.json", [creatorLayer("kenny-trial-2")]);
        ...
        outputs: [
          {
            ...
            resolved: (await load(scene([creatorLayer("kenny-trial-2")]), { allowTrialCreator: true }))
              .resolved,
          },
        ],
      }),
