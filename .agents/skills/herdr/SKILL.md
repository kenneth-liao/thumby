---
name: herdr
description: Control Herdr, a terminal multiplexer for coding agents. Use only when the user explicitly mentions Herdr, loaded local context selects it, or an active workflow requires it to inspect or control panes, tabs, workspaces, commands, or another agent. Do not use merely because a task could benefit from a background terminal, delegation, or parallel work. Requires HERDR_ENV=1.
---
...
When a workflow supplies a stable orchestrator name, use the same value for the current agent name and pane and tab labels:

```bash
herdr agent rename "$HERDR_PANE_ID" orchestrator-123
herdr pane rename "$HERDR_PANE_ID" orchestrator-123
herdr tab rename "$HERDR_TAB_ID" orchestrator-123
```
...
An available shell pane must be at its interactive prompt, with the shell itself in the foreground and no foreground command, editor, or agent running. Start a supported agent in that pane with a useful unique name. When a workflow supplies a stable worker name, use the same value for the agent name and pane label so the worker is visible in the layout:
...
Use the kind requested by the user or selected in loaded local context. Run `herdr agent` to inspect the installed kind list and options. Pass native agent arguments only after `--`:

```bash
herdr agent start reviewer --kind codex --pane <returned-pane-id> -- <agent-args...>
```

When a workflow supplies a role binding, translate its model and thinking/effort level with that kind's native flags. Resolve unknown flags from `<kind> --help`; do not guess them.

For `--kind pi`, require the model as an exact `<provider>/<model>` pair. A bare model name is not a valid binding because multiple providers can expose the same model. Verify a configured pair with `pi --list-models <provider>/<model>` before starting the agent. If only a model name is known, inspect its available providers with `pi --list-models <model>`. Never infer the provider; if there is no exact available match or the binding remains ambiguous, ask the user to confirm it.
